...
`quartz-lang-type-dock` is centered on compilers work in Julia. The technical target is: Create a Julia reference implementation for type workflows, centered on state machine modeling, transition tables, and invalid-transition tests.
...
The score is deterministic. The current threshold is 153. Risk has a penalty of 4, latency has a penalty of 4, and weight contributes a bonus of 2.
